...
public class Abstr extends Phase {
	...
	public static class Logger implements AST.Visitor<Object, String> {
		...
		@Override
		public Object visit(AST.UntilStmt untilStmt, String elemClassName) {
			if (logger == null)
				return null;
			logger.begElement("astnode");
			safeLocation(untilStmt.location());
			logger.addAttribute("id", Integer.toString(untilStmt.id));
			logger.addAttribute("label", untilStmt.getClass().getSimpleName());
			safeAccept(untilStmt.condExpr, null);
			safeAccept(untilStmt.stmts, "Nodes<Stmt>");
			for (AST.Visitor<?, ?> subvisitor : subvisitors)
				untilStmt.accept(subvisitor, null);
			logger.endElement();
			return null;
		}
		// ----- Types -----
