...
prim returns [AST.Expr ast]
	: LPARAN expr___ RPARAN {$ast = $expr___.ast; $ast.relocate(loc($LPARAN, $RPARAN));}
...
expr___ returns [AST.Expr ast]
	: exprEntry3 {$ast = $exprEntry3.ast;};


exprEntry3 returns [AST.Expr ast]
	: e=exprEntry3 OR expr2nd {$ast = new AST.BinExpr(loc($e.ast, $expr2nd3.ast), AST.BinExpr.Oper.OR, $e.ast, $expr2nd3.ast);}
	| e=exprEntry3 OR LPARAN expr2nd RPARAN {$ast = new AST.BinExpr(loc($e.ast, $expr2nd3.ast), AST.BinExpr.Oper.OR, $e.ast, $expr2nd3.ast);}
	| expr2nd3 {$ast = $expr2nd3.ast;}
    ;

expr2nd3 returns [AST.Expr ast]
	: e=expr2nd3 AND expr3rd3 {$ast = new AST.BinExpr(loc($e.ast, $expr3rd3.ast), AST.BinExpr.Oper.AND, $e.ast, $expr3rd3.ast);}
	| e=expr2nd3 AND LPARAN expr3rd3 RPARAN {$ast = new AST.BinExpr(loc($e.ast, $expr3rd3.ast), AST.BinExpr.Oper.AND, $e.ast, $expr3rd3.ast);}
	| expr3rd3 {$ast = $expr3rd3.ast;}
    ;


expr3rd3 returns [AST.Expr ast]
	: a=expr4th3 EQU b=expr4th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.EQU, $a.ast, $b.ast);}
	| a=expr4th3 NEQ b=expr4th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.NEQ, $a.ast, $b.ast);}
	| a=expr4th3 LT b=expr4th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.LTH, $a.ast, $b.ast);}
	| a=expr4th3 GT b=expr4th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.GTH, $a.ast, $b.ast);}
	| a=expr4th3 LEQ b=expr4th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.LEQ, $a.ast, $b.ast);}
	| a=expr4th3 GEQ b=expr4th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.GEQ, $a.ast, $b.ast);}
	| expr4th3 {$ast = $expr4th3.ast;}
	| a=expr4th3 EQU LPARAN b=expr4th3 RPARAN{$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.EQU, $a.ast, $b.ast);}
	| a=expr4th3 NEQ LPARAN b=expr4th3 RPARAN{$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.NEQ, $a.ast, $b.ast);}
	| a=expr4th3 LT LPARAN b=expr4th3 RPARAN{$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.LTH, $a.ast, $b.ast);}
	| a=expr4th3 GT  LPARAN b=expr4th3 RPARAN{$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.GTH, $a.ast, $b.ast);}
	| a=expr4th3 LEQ LPARAN b=expr4th3 RPARAN{$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.LEQ, $a.ast, $b.ast);}
	| a=expr4th3 GEQ LPARAN b=expr4th3 RPARAN{$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.GEQ, $a.ast, $b.ast);}
    ;


expr4th3 returns [AST.Expr ast]
	: a=expr4th3 PLUS b=expr5th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.ADD, $a.ast, $b.ast);}
	| a=expr4th3 PLUS LPARAN b=expr5th3 RPARAN {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.ADD, $a.ast, $b.ast);}
	| a=expr4th3 MINUS b=expr5th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.SUB, $a.ast, $b.ast);}
	| a=expr4th3 MINUS LPARAN b=expr5th3 RPARAN {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.SUB, $a.ast, $b.ast);}

	| expr5th3 {$ast = $expr5th3.ast;};



expr5th3 returns [AST.Expr ast]
	: a=expr5th3 MUL b=expr6th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.MUL, $a.ast, $b.ast);}
	| a=expr5th3 DIV b=expr6th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.DIV, $a.ast, $b.ast);}
	| a=expr5th3 MOD b=expr6th3 {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.MOD, $a.ast, $b.ast);}
	| a=expr5th3 MUL LPARAN b=expr6th3 RPARAN {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.MUL, $a.ast, $b.ast);}
	| a=expr5th3 DIV LPARAN b=expr6th3 RPARAN {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.DIV, $a.ast, $b.ast);}
	| a=expr5th3 MOD LPARAN b=expr6th3 RPARAN {$ast = new AST.BinExpr(loc($a.ast, $b.ast), AST.BinExpr.Oper.MOD, $a.ast, $b.ast);}
	| expr6th3 {$ast = $expr6th3.ast;} 
	;

expr6th3 returns [AST.Expr ast]
	: expr7th3 {$ast = $expr7th3.ast;}
	| PLUS expr6th3 {$ast = new AST.PfxExpr(loc($PLUS, $expr6th3.ast), AST.PfxExpr.Oper.ADD, $expr6th3.ast);}
	| MINUS expr6th3 {$ast = new AST.PfxExpr(loc($MINUS, $expr6th3.ast), AST.PfxExpr.Oper.SUB, $expr6th3.ast);}
	| EXCL expr6th3 {$ast = new AST.PfxExpr(loc($EXCL, $expr6th3.ast), AST.PfxExpr.Oper.NOT, $expr6th3.ast);}
	| POW expr6th3 {$ast = new AST.PfxExpr(loc($POW, $expr6th3.ast), AST.PfxExpr.Oper.PTR, $expr6th3.ast);}
	;

expr7th3 returns [AST.Expr ast]
	: e=expr7th3 POW { $ast = new AST.SfxExpr(loc($e.ast, $POW), AST.SfxExpr.Oper.PTR, $e.ast);}
	| e=expr7th3 DOT IDENTIFIER { $ast = new AST.CompExpr(loc($e.ast, $IDENTIFIER), $e.ast, $IDENTIFIER.getText());} 
	| e=expr7th3 LPARAN exprs_ RPARAN { $ast = new AST.CallExpr(loc($e.ast, $RPARAN), $e.ast, $exprs_.ast);}
	| e=expr7th3 LBRCKT expr RBRCKT { $ast = new AST.ArrExpr(loc($e.ast, $RBRCKT), $e.ast, $expr.ast);}
	| prim3 {$ast = $prim3.ast;}
	;


prim3 returns [AST.Expr ast]
	: LBRACE expr___ COLON type1 RBRACE {$ast = new AST.CastExpr(loc($LBRACE, $RBRACE), $type1.ast, $expr___.ast);}
	| exprEnd {$ast = $exprEnd.ast;}
	| IDENTIFIER {$ast = new AST.NameExpr(loc($IDENTIFIER), $IDENTIFIER.getText());}
	| SIZEOF type1 {$ast = new AST.SizeExpr(loc($SIZEOF, $type1.ast), $type1.ast);}
	;
